...
from config import (
    ...
    GOSSIP_INTERVAL, MESSAGE_RETRY_COUNT, MESSAGE_RETRY_DELAY,
    get_local_ip
)
...
from message import (
    ...
    create_coordinator, create_gossip
)
import random
...
class Node:
            ...
            with self._nodes_lock:
                for nid in self._known_nodes:
                    if nid != self.node_id:
                        ...
                        sent = self.send_with_ack_retry(ledger_sync_msg, nid, attempts=MESSAGE_RETRY_COUNT, timeout=MESSAGE_RETRY_DELAY)
                        if not sent:
    ...
    def _announce_join(self):
        ...
        self.multicast.send_reliable(join_msg)
    ...
    def _announce_leave(self):
        ...
        try:
            ...
            if coordinator_id is not None and coordinator_id != self.node_id:
                ...
                sent = self.send_with_ack_retry(state_msg, coordinator_id, attempts=MESSAGE_RETRY_COUNT, timeout=MESSAGE_RETRY_DELAY)
                if sent:
                    logger.debug(f"Node {self.node_id}: Sent ledger state to coordinator {coordinator_id} before leave")
                else:
    ...
    def _on_multicast_message(self, data, addr):
        ...
        try:
            ...
            # without updating vector clock (they are liveness probes, not application events).
    ...
    def _handle_join(self, message):
        ...
        if self.election.is_coordinator():
            ...
            sent = self.send_with_ack_retry(join_response, new_node_id, attempts=MESSAGE_RETRY_COUNT, timeout=MESSAGE_RETRY_DELAY)
            if sent:
            ...
            # Also send LEDGER_SYNC for redundancy (reliable unicast)
            ledger_sync_msg = create_ledger_sync(self.node_id, clock, ledger_state)
            sent2 = self.send_with_ack_retry(ledger_sync_msg, new_node_id, attempts=MESSAGE_RETRY_COUNT, timeout=MESSAGE_RETRY_DELAY)
            if not sent2:
            ...
            self.multicast.send_reliable(coord_msg)
    ...
    def _handle_join_response(self, message):
        ...
        - Ledger state: Restored balance and transaction history
        """
        ...
        if ledger_state:
            ...
            try:
                ...
                if msg_id and coordinator_id is not None:
                    ...
                    logger.debug(f"Node {self.node_id}: Sent ACK for JOIN_RESPONSE msg_id={msg_id}")
            except Exception:
                pass
    ...
    def _handle_trade_response(self, message):
        ...
        if accepted:
            ...
            self.unicast.send_with_retry(confirm, sender_id, retries=MESSAGE_RETRY_COUNT)
    ...
    def _handle_ledger_sync(self, message):
        ...
        if msg_id:
            try:
                ...
                logger.debug(f"Node {self.node_id}: Sent ACK for msg_id={msg_id}")
            except Exception as e:
    ...
    def _handle_state_request(self, message):
        ...
        try:
            ...
            ledger_state = self.ledger.get_state()
            state_msg = create_ledger_sync(self.node_id, clock, ledger_state)
            sent = self.send_with_ack_retry(state_msg, requester_id, attempts=MESSAGE_RETRY_COUNT, timeout=MESSAGE_RETRY_DELAY)
            if sent:
                logger.debug(f"Node {self.node_id}: Sent LEDGER_SYNC to Node {requester_id}")
            else:
                logger.warning(f"Node {self.node_id}: Sent LEDGER_SYNC to Node {requester_id} but no ACK received")
        except Exception as e:
    ...
    def send_with_ack_retry(self, message, target_node_id, attempts=None, timeout=None):
        """
        ...
        """
        if attempts is None:
            attempts = MESSAGE_RETRY_COUNT
        if timeout is None:
            timeout = MESSAGE_RETRY_DELAY
        ...
        backoff = MESSAGE_RETRY_DELAY / 5  # Small backoff between attempts
    ...
    def _on_coordinator_change(self, new_coordinator_id):
        ...
        try:
            if new_coordinator_id is not None and new_coordinator_id != self.node_id:
                ...
                sent = self.send_with_ack_retry(msg, new_coordinator_id, attempts=MESSAGE_RETRY_COUNT, timeout=MESSAGE_RETRY_DELAY)
                if sent:
                    logger.debug(f"Node {self.node_id}: Sent ledger state to new coordinator Node {new_coordinator_id}")
                else:
    ...
    def _bootstrap_registry(self, timeout_per_request=0.5):
        ...
        for nid in peers:
            try:
                ...
                sent = self.unicast.send_with_retry(req, nid, retries=MESSAGE_RETRY_COUNT)
                if not sent:
...
Available Commands:
  ...
  quit       - Shutdown node gracefully
    ...
    def _print_nodes(self):
        """Print list of known nodes (uses heartbeat tracking for consistency with status)."""
        active = self.heartbeat.get_active_nodes()
        suspected = self.heartbeat.get_suspected_nodes()
        # Combine active and suspected to get all known nodes (same source as status command)
        nodes = sorted(active | suspected)
        coordinator = self.election.get_coordinator()
