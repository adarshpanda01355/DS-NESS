...
from config import UNICAST_PORT_BASE, BUFFER_SIZE, MESSAGE_RETRY_COUNT, MESSAGE_RETRY_DELAY, get_local_ip
...
class UnicastHandler:
    ...
    def __init__(self, node_id, port=None, host=None, peer_addresses=None):
        ...
        Args:
            ...
            host: Optional host IP to bind to (default: auto-detected via get_local_ip())
        ...
        # Host address - auto-detect for multi-device deployment
        if host is not None:
            self.host = host
        else:
    ...
    def _setup_socket(self):
        ...
        # Bind to this node's port on the detected/configured host address
        # Bind to 0.0.0.0 (all interfaces) to ensure we can receive from any interface
        # and allow the OS to route outgoing packets via the appropriate interface.
        # This fixes [Errno 49] Can't assign requested address on some systems.
    ...
    def get_node_address(self, target_node_id):
        ...
        Falls back to this node's host if not found.
        ...
        # Use peer address if available, otherwise use this node's host address
